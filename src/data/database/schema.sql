...
CREATE TABLE IF NOT EXISTS `delivery_batches` (
  ...
  `status` VARCHAR(50) NOT NULL COMMENT '''Ready for Delivery'', ''Delivered''',
  `userId` VARCHAR(191) NULL DEFAULT NULL,
  `info` TEXT NULL DEFAULT NULL,
  `obs` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_delivery_batches_user` (`userId` ASC),
  CONSTRAINT `fk_delivery_batches_user`
    FOREIGN KEY (`userId`)
    REFERENCES `users` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `delivery_batch_items` (
  ...
  `obs` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_delivery_items_batch` (`batchId` ASC),
  INDEX `fk_delivery_items_book` (`bookId` ASC),
  CONSTRAINT `fk_delivery_items_batch`
    FOREIGN KEY (`batchId`)
    REFERENCES `delivery_batches` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_delivery_items_book`
    FOREIGN KEY (`bookId`)
    REFERENCES `books` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
