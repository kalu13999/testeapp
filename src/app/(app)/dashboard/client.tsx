    ...
    return (
        <div className="flex flex-col gap-6">
            <h1 className="font-headline text-3xl font-bold tracking-tight">
                {currentUser.role === 'Admin' ? 'Admin Dashboard' : (selectedProjectId ? `${projects.find(p=>p.id === selectedProjectId)?.name} Dashboard` : (currentUser.role === 'Client' ? 'Client Dashboard' : 'My Dashboard'))}
            </h1>
            {renderDashboard()}
        </div>
    )
